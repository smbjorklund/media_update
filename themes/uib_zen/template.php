...
/**
 * Overrides theme_link_formatter_link_separate()
 * [Link module]
 *
 * @param $vars
 ...
 *  Markup for the link.
 */
function uib_zen_link_formatter_link_separate($vars) {
  ...
  if ($vars['field']['field_name'] == 'field_uib_user_url' && $vars['field']['entity_type'] == 'user') {
    // Use default link title for user profile urls when title is an url, like when title is empty.
    $title_as_url = parse_url($vars['element']['title']);
    if (!empty($title_as_url['scheme'])) {
      $title = t('Home page');
    }
  }
